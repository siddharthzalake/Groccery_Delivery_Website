   ...
   try {
    ...
    if (paymentOption === "COD") {
      const response = await axios.post("/api/order/cod", {
        ...
        address: selectedAddress._id   // ✅ REQUIRED
      });
        ...
        <div className="flex flex-col md:flex-row mt-12">
            <div className='flex-1 max-w-4xl'>
                ...
                <button onClick={() => { navigate("/products"); scrollTo(0,0);}} className="group cursor-pointer flex items-center mt-8 gap-2 text-primary-dull font-medium">
                    <img src={assets.arrow_right_icon_colored} className="group-hover:-translate-x-1 transition" alt="arrow icon" />
                    Continue Shopping
                </button>
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                {addresses.map((address, index) => (
                                   <p
                                     key={index}   // ✅ THIS LINE FIXES THE WARNING
                                     onClick={() => {
